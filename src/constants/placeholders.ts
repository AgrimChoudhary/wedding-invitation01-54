...
// Photos with dynamic data support - Only show if photos exist
export const PHOTOS = dynamicData.photos && dynamicData.photos.length > 0 ?
  dynamicData.photos.map((photo, index) => ({
    [`PHOTO_${index + 1}`]: photo
  })) : [];

// Check if we should show photo gallery
export const SHOW_PHOTO_GALLERY = PHOTOS.length > 0;

// Main venue information with dynamic data support
export const VENUE_NAME = dynamicData.venueName || "";
export const VENUE_ADDRESS = dynamicData.venueAddress || "";
export const VENUE_MAP_LINK = dynamicData.venueMapLink || "";

// Check if we should show venue details
export const SHOW_VENUE_DETAILS = VENUE_NAME || VENUE_ADDRESS;
...
// Check if we should show contact details
export const SHOW_CONTACT_DETAILS = CONTACTS.length > 0;
