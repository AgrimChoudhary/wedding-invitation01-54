...
const WishingWall: React.FC<WishingWallProps> = ({ 
  ...
  useEffect(() => {
    const handleWishAdded = (data: any) => {
      console.log('New wish received from platform:', data);
      setWishes(prev => [data.wish, ...prev]);
    };

    const handleWishLikedUpdate = (data: any) => {
      console.log('Wish like updated from platform:', data);
      setWishes(prev => prev.map(wish => 
        wish.id === data.wishId 
          ? { ...wish, likes: data.newLikeCount }
          : wish
      ));
    };

    const handleWishDeleted = (data: any) => {
      console.log('Wish deleted from platform:', data);
      setWishes(prev => prev.filter(wish => wish.id !== data.wishId));
    };

    // Register listeners for platform messages
    iframeMessenger.onMessage('WISH_ADDED', handleWishAdded);
    iframeMessenger.onMessage('WISH_LIKED_UPDATE', handleWishLikedUpdate);
    iframeMessenger.onMessage('WISH_DELETED', handleWishDeleted);

    // Cleanup listeners on unmount
    return () => {
      iframeMessenger.removeListener('WISH_ADDED', handleWishAdded);
      iframeMessenger.removeListener('WISH_LIKED_UPDATE', handleWishLikedUpdate);
      iframeMessenger.removeListener('WISH_DELETED', handleWishDeleted);
    };
  }, []);
    ...
    <div className="py-10 md:py-12 px-4 relative z-10 bg-gradient-to-b from-transparent via-maroon/5 to-transparent">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10 md:mb-12 bg-white/80 backdrop-blur-sm rounded-2xl p-6 mx-auto max-w-2xl border border-gold-light/30 shadow-lg">
          ...
          <h2 className={cn(
            "font-cormorant text-maroon font-bold mb-4 drop-shadow-sm",
            isInIframe ? "text-3xl md:text-4xl" : "text-4xl md:text-5xl"
          )}>
            Wedding Wishes
          </h2>
          
          <p className="text-gold-light/90 font-opensans text-base md:text-lg mb-2 font-medium">
            Heartfelt blessings from our loved ones
          </p>
          <p className="text-maroon/80 font-cormorant text-xl md:text-2xl font-semibold">
            {wishes.length} beautiful {wishes.length === 1 ? 'wish' : 'wishes'} shared
          </p>
        </div>

        {/* Enhanced Carousel with single card sliding */}
        <div className="relative mb-10 md:mb-12">
          {wishes.length > 0 ? (
            <>
              {/* Carousel Container - Single card view */}
              <div 
              ...
              {totalSlides > 1 && (
                <div className="flex items-center justify-center mt-8 space-x-6">
                  <button
                    ...
                    className={cn(
                      "w-14 h-14 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center transition-all duration-300 shadow-xl border-2 border-gold-light/30",
                      canSlideLeft && !isTransitioning
                        ? "hover:scale-110 hover:shadow-gold-light/40 active:scale-95 hover:border-gold-light/60"
                  ...
                  <div className="flex space-x-3 bg-white/60 backdrop-blur-sm rounded-full px-4 py-2 border border-gold-light/30">
            ...
            <div className="text-center py-16 bg-white/60 backdrop-blur-sm rounded-2xl border border-gold-light/30 shadow-lg">
        ...
        {allowWishPosting && (
          <div className="text-center">
            <button
              ...
              className={cn(
                "bg-gold-gradient text-maroon font-bold rounded-full hover:scale-105 transition-all duration-300 shadow-xl hover:shadow-gold-light/40 inline-flex items-center relative overflow-hidden group border-2 border-white/50 backdrop-blur-sm",
            ...
            >
              ...
              <span className="relative z-10 font-semibold">Post Your Wish</span>
